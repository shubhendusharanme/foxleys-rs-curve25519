...
const prefixKeyInPublicKey = (pubKey) => {
...
function unclampEd25519PrivateKey(clampedSk) {
  ...
  // Fix first byte (restore bits removed by Ed25519 clamping)
  unclampedSk[0] |= 6
...
const getPublicFromPrivateKey = (privKey) => {
...
const generateKeyPair = () => {
  try {
    const { publicKey: publicDer, privateKey: privateDer } = nodeCrypto.generateKeyPairSync('x25519', {
      publicKeyEncoding: { format: 'der', type: 'spki' },
      privateKeyEncoding: { format: 'der', type: 'pkcs8' },
    })

    const pubKey = publicDer.slice(PUBLIC_KEY_DER_PREFIX.length, PUBLIC_KEY_DER_PREFIX.length + 32)

    const privKey = privateDer.slice(PRIVATE_KEY_DER_PREFIX.length, PRIVATE_KEY_DER_PREFIX.length + 32)
    ...
    // fallback to curve25519-js pure implementation
...
const verifySignature = (pubKey, msg, sig, isInit) => {
  pubKey = scrubPubKeyFormat(pubKey)
